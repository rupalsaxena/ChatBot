...
class Embeddings:
    ...
    def apply_embedding(self, ent, pred_id):
        try:
            head = self.entity_emb[self.ent2id[self.lbl2ent[ent]]]
            pred = self.relation_emb[self.rel2id[WDT[pred_id]]]
            lhs = head + pred
            dist = pairwise_distances(lhs.reshape(1, -1), self.entity_emb).reshape(-1)
            most_likely = dist.argsort()
            df = pd.DataFrame([(self.id2ent[idx][len(WD):], self.ent2lbl[self.id2ent[idx]], dist[idx], rank+1) for rank, idx in enumerate(most_likely[:2])], columns=('Entity', 'Label', 'Score', 'Rank'))
            return df["Label"].values[0]
        except:
            print("embedings failing")
            return "Answer not found in embedding"
    
    def find_similar_entities(self, label):
        try:
            ent = self.ent2id[self.lbl2ent[label]] #Q102225
